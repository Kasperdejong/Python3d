...
class PlantSystem:
    ...
    def load_plant_data(self):
        ...
        try:
            if not os.path.exists(json_path):
                print("❌ JSON/plants.json not found!")
    ...
    def interact(self, particle):
        ...
        if slot_idx < 0 or slot_idx >= self.num_slots: return False
        ...
        if particle.element_type == "Water":
            if particle.y >= self.h - 15:
                ...
                # It will take longer to reach max height because target_h is huge now
                if current_h < target_h:
                    self.plant_heights[slot_idx] += 3.0
                return True 
            ...
            if plant_h > 0:
                ...
                if particle.y > plant_top:
                    ...
                    if self.plant_heights[slot_idx] < 0: 
                        self.plant_heights[slot_idx] = 0
                        self.plant_types[slot_idx] = -1
                    return True 
        return False

    def draw(self, frame):
        # Draw Grass/Soil Layer
...
class Particle:
    def __init__(self, x, y, element_type, velocity=None):
        ...
        if self.element_type == "Fire":
            if velocity:
                self.vx = velocity[0] + random.uniform(-2, 2)
                self.vy = velocity[1] + random.uniform(-2, 2)
        ...
        elif self.element_type == "Leaf":
            self.vx = random.uniform(-1, 1); self.vy = random.uniform(-2, -5) 
            self.size = random.randint(4, 7); self.decay = 0.02 
            self.angle = random.uniform(0, 360); self.sway_speed = random.uniform(0.1, 0.3)
            self.tick = random.random() * 10 
        elif self.element_type == "Wind":
            self.center_x = x; self.center_y = y
            self.angle = random.uniform(0, math.pi * 2); self.radius = random.uniform(25, 45) 
            self.speed = random.uniform(0.2, 0.4); self.size = random.randint(1, 2)
            self.decay = 0.08
        elif self.element_type == "Rock":
            self.vx = random.uniform(-1, 1); self.vy = random.uniform(3, 8) 
            self.size = random.randint(5, 10); self.decay = 0.04
            self.shape_points = []; num_points = random.randint(3, 5)
            for _ in range(num_points):
                rx = random.randint(-self.size, self.size); ry = random.randint(-self.size, self.size)
                self.shape_points.append([rx, ry])
            grey = random.randint(100, 180); self.color = (grey, grey, grey)

    def update(self):
        self.life -= self.decay
        if self.element_type != "Wind":
            self.x += self.vx; self.y += self.vy

        if self.element_type == "Water": self.vy += 0.5 
        elif self.element_type == "Rock": self.vy += 0.5 
        elif self.element_type == "Leaf":
            self.tick += self.sway_speed; self.vx = math.sin(self.tick) * 2 
            self.angle += 5 
        elif self.element_type == "Wind":
            self.angle += self.speed; self.radius += 0.5 
    ...
    def draw(self, frame):
        ...
        if self.element_type == "Fire":
            ...
            green_val = int(150 + (self.life * 100))
            color = (50, green_val, 50) 
            box_size = (int(self.size * 2), int(self.size))
            cv2.ellipse(frame, (ix, iy), box_size, self.angle, 0, 360, color, -1)
        elif self.element_type == "Wind":
            tail_angle = self.angle - 0.4 
            tx = int(self.center_x + math.cos(tail_angle) * self.radius)
            ty = int(self.center_y + math.sin(tail_angle) * self.radius)
            color = (230, 230, 230) 
            cv2.line(frame, (ix, iy), (tx, ty), color, self.size)
        elif self.element_type == "Rock":
            pts = np.array([[int(self.x + px), int(self.y + py)] for px, py in self.shape_points], np.int32)
...
def background_thread():
    ...
    crown_indices = [54, 103, 67, 109, 10, 338, 297, 332, 284]
    chin_indices = [148, 152, 377] 
    ...
    while True:
        ...
        if results.left_hand_landmarks:
            ...
            if is_hand_open(landmarks):
                for lm in landmarks: particles.append(Particle(*get_coords(lm), "Water"))
        ...
        if results.face_landmarks:
            ...
            # Water vs Head
            pt_left = get_coords(face_lms[crown_indices[0]]); pt_right = get_coords(face_lms[crown_indices[-1]]); pt_center = get_coords(face_lms[10])
            crown_y = pt_center[1]; crown_min_x = min(pt_left[0], pt_right[0]); crown_max_x = max(pt_left[0], pt_right[0])
            
            water_particles = [p for p in particles if p.element_type == "Water"]
            for wp in water_particles:
                if wp.life > 0:
                    if crown_min_x < wp.x < crown_max_x and abs(wp.y - crown_y) < 40:
                        wp.life = 0; particles.append(Particle(wp.x, crown_y, "Leaf"))

            # Elements
            for i in range(len(crown_indices) - 1):
                pt1 = get_coords(face_lms[crown_indices[i]]); pt2 = get_coords(face_lms[crown_indices[i+1]])
                cv2.line(frame, pt1, pt2, (100, 255, 100), 2)
                if random.random() < 0.3: particles.append(Particle(pt1[0], pt1[1], "Leaf"))

            if len(face_lms) > 473:
                pupil_right = get_coords(face_lms[468]); pupil_left = get_coords(face_lms[473])
                for _ in range(2): 
                    particles.append(Particle(pupil_right[0], pupil_right[1], "Wind"))
                    particles.append(Particle(pupil_left[0], pupil_left[1], "Wind"))

            p_left = get_coords(face_lms[chin_indices[0]]); p_mid = get_coords(face_lms[chin_indices[1]]); p_right = get_coords(face_lms[chin_indices[2]])
            stone_color = (150, 150, 150)
            cv2.line(frame, p_left, p_mid, stone_color, 4); cv2.line(frame, p_mid, p_right, stone_color, 4)
            for idx in chin_indices:
                pt = get_coords(face_lms[idx])
                if random.random() < 0.4: particles.append(Particle(pt[0], pt[1], "Rock"))

        # 3. INTERACTION
        fire_particles = [p for p in particles if p.element_type == "Fire"]
        leaf_particles = [p for p in particles if p.element_type == "Leaf"]
        if fire_particles and leaf_particles:
            for fire in fire_particles:
                if fire.y < height / 2:
                    for leaf in leaf_particles:
                        if leaf.life > 0:
                            dx = fire.x - leaf.x; dy = fire.y - leaf.y
                            if (dx*dx + dy*dy) < 400: leaf.life = 0
...
<body>
    <div id="video-container">
        <div class="overlay-text"><b>Left Hand:</b> Water (Mega Growth)<br><b>Right Hand:</b> Flamethrower</div>
